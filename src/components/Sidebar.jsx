    ...
    <>
      ...
      <aside className="hidden lg:flex w-64 shrink-0 bg-white dark:bg-[#1a1210] border-r border-[#e5e0d8] dark:border-[#3d2f2b] flex-col">
            ...
            return (
              <div key={s.num} className="flex gap-3 items-start">
                <div className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium shrink-0 transition-colors ${done || active ? 'bg-[#8B3714] text-white' : 'border-2 border-[#d5cfc8] dark:border-[#3d2f2b] text-[#7a6f68] dark:text-[#5c4f4a]'
                    }`}>
        ...
        <div className="mt-auto p-5 border-t border-[#e5e0d8] dark:border-[#3d2f2b]">
          ...
          <p className="text-[10px] text-[#b0a898] dark:text-[#5c4f4a] mt-2">Created by Yuri L.</p>
        </div>
      </aside>
          ...
          {STEPS.map((s) => {
            ...
            return (
              <div key={s.num} className="flex flex-col items-center gap-1">
                <div
                  title={s.label}
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${done || active ? 'bg-[#8B3714] text-white' : 'border-2 border-[#d5cfc8] dark:border-[#3d2f2b] text-[#7a6f68] dark:text-[#5c4f4a]'
                    }`}
                >
                  {done ? <CheckIcon /> : s.num}
                </div>
        ...
        {STEPS.map((s) => {
          ...
          return (
            <div key={s.num} className="flex-1 flex flex-col items-center justify-center py-2 gap-1">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-colors ${done || active ? 'bg-[#8B3714] text-white' : 'border border-[#d5cfc8] dark:border-[#3d2f2b] text-[#b0a898] dark:text-[#5c4f4a]'
                }`}>
                {done ? <CheckIcon /> : s.num}
              </div>
