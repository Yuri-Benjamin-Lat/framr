            ...
            <div key={s.num} className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium shrink-0 transition-colors ${
                    ...
                    : 'border-2 border-[#d5cfc8] text-[#7a6f68]'
                }`}>
                  {done
              ...
              <div className={i < STEPS.length - 1 ? 'pb-7' : ''}>
                <p className={`text-sm font-medium leading-none mb-0.5 ${active ? 'text-[#1a1614]' : 'text-[#7a6f68]'}`}>
                  {s.label}
                </p>
                {active && (
                  <p className="text-xs text-[#8B3714]">{s.sub}</p>
                )}
              </div>
            </div>
          )
