...
export default function SaveShareStep({ format, photos, filter, frameColor, frameStyle, layers = [], stickerOverflow = false, onRestart, onBack }) {
  ...
  useEffect(() => {
    compositePhoto({ photos, format, filter, frameColor, frameStyle, layers, stickerOverflow }).then(url => {
      setOutputUrl(url)
      setGenerating(false)
    })
  }, [])
  ...
  return (
    <div className="flex flex-col md:flex-row md:h-full">
      ...
      <div className="flex-1 flex items-center justify-center bg-[#f5f0ea] dark:bg-[#191210] p-6 md:p-10 min-h-[40vh] md:min-h-0 md:overflow-auto">
        {outputUrl && (
          <img src={outputUrl} alt="Your print" className="max-w-full max-h-full object-contain" />
        )}
      </div>
